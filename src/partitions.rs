...
use crate::error::OtaInternalError::{PartitionFoundTwice, PartitionNotFound, StorageOpError};
use embedded_storage::Storage;
use esp_partition_table::{PartitionEntry, PartitionTable, PartitionType};

/// Find partition entry by type
pub fn find_partition_type<S: Storage>(
    storage: &mut S,
    typ: PartitionType,
    ...
    for entry in table.iter_storage(storage, false) {
        let entry = entry.map_err(StorageOpError)?;
        if entry.type_ == typ {
